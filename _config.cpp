...
void logError(String e) {
	Serial.println(e);
	info["errorLog"] = info["errorLog"].as<String>() + e + +" \n\r ";
}
...
void saveConfigToEEPROM() {
	...
	if (jsonSize > JSON_MAX_SIZE) {
		logError("Erreur : Le JSON est trop grand pour l'EEPROM !\n\r");
		return;
	}
...
bool loadConfigFromEEPROM() {
	...
	if (error) {
		logError("Erreur de désérialisation du JSON !\n\r");
		return false;
	}
...
void initConfig() {
	...
	if (!EEPROM.begin(EEPROM_SIZE)) {
		logError("Erreur d'initialisation de l'EEPROM !\n\r");
		return;
	}
	...
	if (isFirstBoot()) {
		...
		if (loadConfigFromEEPROM()) {
			...
			logError("Erreur lors du chargement du JSON depuis l'EEPROM !\n\r");
		}
	}
}
...
void writeConfig(String key, bool value, bool force) {
	if (force || config[key].as<bool>() != value) {
		config[key] = value;
		configUpdated(key);
		saveConfigToEEPROM();
	}
...
void writeConfig(String key, int value, bool force) {
	if (force || config[key].as<int>() != value) {
		config[key] = value;
		configUpdated(key);
		saveConfigToEEPROM();
	}
...
void writeConfig(String key, float value, bool force) {
	if (force || config[key].as<float>() != value) {
		config[key] = value;
		configUpdated(key);
		saveConfigToEEPROM();
	}
...
void writeConfig(String key, String value, bool force) {
	if (force || config[key].as<String>() != value) {
		config[key] = value;
		configUpdated(key);
		saveConfigToEEPROM();
	}
